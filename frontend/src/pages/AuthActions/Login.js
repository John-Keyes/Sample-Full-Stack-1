...
const Login = () => {
    ...
    const {authInfo, isLoading, errors} = useSelector(state => state.auth);
    ...
    const SubmitLogin = () => {
        if(!isLoading) {
            dispatch(SaveProfile("GET", `login/${authInfo.studentID}`, {email: email.toLowerCase(), password: password}));
            if(!errors) {
                navigate("Profile");
            }
        }
    ...
    return (
        <div className="formContainer">
            ...
            />
            <Button additionalClassNames={"signButton"} toggle={() => SubmitLogin()}>
